...
public class MultiPeer: NSObject {
    ...
    /** Device's name */
    let devicePeerID = MCPeerID(displayName:  (UIDevice.current.identifierForVendor?.description)!)
    ...
    public func sendstringoptimize(object: String) {
        if isConnected {
            do {
                let item = NSKeyedArchiver.archivedData(withRootObject: object)
